...
export default function App() {
  ...
  const handleLogin = async (role: UserRole) => {
    try {
      let name = 'Даукеш А.';
      if (role === 'teacher') name = 'Учитель Даукеш';
      if (role === 'admin') name = 'Админ Даукеш';
      if (role === 'parent') name = 'Родитель Даукеша';
      
      const user = await dataService.login(name, 'amangereev.daukesh@gmail.com', role);
      setProfile(user);
    } catch (error) {
