...
export function ParentDashboard({ profile, onLogout, theme, toggleTheme }: ParentDashboardProps) {
  ...
  useEffect(() => {
    // In a real app, we would fetch grades for the specific child
    // For mock purposes, we use the current user's UID or a mock student UID
    const unsubGrades = dataService.subscribeToGrades('student_123', (data) => {
      setGrades(data);
    });
