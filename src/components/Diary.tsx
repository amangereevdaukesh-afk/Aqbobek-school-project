...
const generateRandomGrades = () => ({
  mon: Math.random() > 0.3 ? Math.floor(Math.random() * 3) + 3 : null,
  tue: Math.random() > 0.3 ? Math.floor(Math.random() * 3) + 3 : null,
  wed: Math.random() > 0.3 ? Math.floor(Math.random() * 3) + 3 : null,
  thu: Math.random() > 0.3 ? Math.floor(Math.random() * 3) + 3 : null,
  fri: Math.random() > 0.3 ? Math.floor(Math.random() * 3) + 3 : null,
});

const calculateAvg = (grades: any) => {
  const vals = Object.values(grades).filter(v => v !== null) as number[];
  if (vals.length === 0) return 0;
  return Number((vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(1));
};

const generateQuarterData = (quarter: number): DiaryEntry[] => {
  return SUBJECTS.map(subject => {
    ...
    return {
      ...
      comment: quarter === 4 ? 'Четверть еще не завершена.' : 'Хорошая работа в этой четверти.'
    };
  });
};

const MOCK_QUARTER_DATA: Record<number, DiaryEntry[]> = {
  1: generateQuarterData(1),
  2: generateQuarterData(2),
  3: generateQuarterData(3),
  4: generateQuarterData(4),
};

const DIARY_DATA = MOCK_QUARTER_DATA[3];
...
export function Diary({ theme = 'dark', profile }: DiaryProps) {
  ...
  const handleQuarterChange = (id: number) => {
    ...
    setTimeout(() => {
      setDiaryData(MOCK_QUARTER_DATA[id] || DIARY_DATA);
      setIsLoading(false);
    }, 400);
  };
